...
**Approval Timeline custom section** (`webapp/ext/`):
- Custom OP section declared in `manifest.json` → `content.body.sections.Timeline` (template `capmap.requestsui.ext.fragment.Timeline`, anchored `After` `RequestItemsFacet`).
- [Timeline.fragment.xml](app/requestsui/webapp/ext/fragment/Timeline.fragment.xml) uses `sap.suite.ui.commons.Timeline` — **must** declare `sap.suite.ui.commons` in `manifest.json` → `sap.ui5.dependencies.libs`.
- The timeline is a **derived list of lifecycle events** (Created → AI Check → Submitted → Approved/Rejected/Cancelled) synthesized from scalar `Requests` fields. No such collection exists in OData, so it is built client-side into a named `JSONModel("timeline")` and bound via `content="{timeline>/events}"`.
- Built by the controller extension [RequestObject.controller.ts](app/requestsui/webapp/ext/controller/RequestObject.controller.ts), registered under `sap.ui.controllerExtensions` → `sap.fe.templates.ObjectPage.ObjectPageController#capmap.requestsui::RequestsObjectPage`. Uses `BaseControllerExtension` + `createExtensionOverrides({ routing: { onAfterBinding } })` to react when the Request is bound. All timeline labels live in i18n (`timeline*` keys, all 3 bundles).
...
### Fiori controller extensions (`webapp/ext/controller/`)

- **`this` inside `createExtensionOverrides` sub-extension hooks is NOT your extension instance**: For hooks nested under `routing` / `editFlow` / `messageHandler` etc. (e.g. `routing: { onAfterBinding }`), the transpiler merges them into FE's *sub-extension* (`override: {...}`), so at runtime `this` is that FE sub-extension — it has `this.base` and `this.getView()` but **NOT** your own instance methods/fields. `this.myMethod()` / `this.myField` resolve to `undefined`. The `MapThis<T,…>` typing claims `this = YourClass` but runtime disagrees. **Fix**: reach the view via `this.base.getView()` (host controller, always available) and put logic in **module-level functions**, not instance methods. Confirmed by inspecting the transpiled `RequestObject-dbg.controller.js`.
- **OData V4 late properties — `requestObject()` cannot fetch unselected fields**: With `autoExpandSelect: true` the model only `$select`s fields rendered by bound controls. `Context.requestObject()` is limited to the binding's `$expand`/`$select`, so any field not shown on the page comes back `undefined`. To read fields not on the UI (e.g. `createdAt`, `approver`, `modifiedBy` for the timeline), use `Context.requestProperty(['field1','field2',…])` — it issues a **late property request** (`GET …/Requests(ID)?$select=field1,field2`) and returns the values in path order. Paths must be primitive structural properties.
- **`@sapui5/types` are ambient module declarations loaded via `typeRoots`**: Never hand-write a `*.d.ts` that re-`declare module "sap/..."` — it *shadows* the real UI5 types (e.g. a local `ControllerExtension.d.ts` stripped `getView`/`byId`). The editor often shows false `Cannot find module 'sap/...'` / missing-method errors when the TS server attaches the file to an inferred project (no `typeRoots`); the CLI `tsc` is authoritative. `.vscode/settings.json` pins the workspace TS SDK. Restart the TS server after deleting/adding ambient `.d.ts` files.
- **ESLint `@sap-ux/fiori-tools/sap-no-global-variable`** flags module-level `const`/`let` (treated as script globals) but not `function` declarations. Keep helper state inside functions (or pass it as args) rather than module-level variables. `DateFormat.getDateTimeInstance(...)` instances are internally cached by UI5, so calling it inside a helper per invocation is cheap.
- **Async hooks assigned to `void`-typed override slots**: `routing.onAfterBinding` is typed `=> void`; assigning an `async` function trips `@typescript-eslint/no-misused-promises`. Keep the hook sync and delegate: `onAfterBinding(...) { void doAsyncWork(...); }`.
